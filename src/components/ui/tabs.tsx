...
function TabsList({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn('inline-flex h-9 items-center justify-center rounded-lg bg-muted/60 p-1 text-muted-foreground gap-px', className)} {...props}>
      {children}
    </div>
  )
}
...
function TabsContent({ className, value, children, ...props }: TabsContentProps) {
  ...
  return (
    <div className={cn('mt-3 animate-fade-in', className)} {...props}>
      {children}
    </div>
  )
}
