...
export default function InventoryDashboardPage() {
  ...
  const sections = [
    { title: t('inventory.stockOverview'), href: '/dashboard/inventory', icon: Package, value: String(stock.length), color: 'bg-blue-500' },
    { title: t('inventory.lowStockAlerts'), href: '/dashboard/inventory', icon: AlertTriangle, value: String(lowStockCount), color: lowStockCount > 0 ? 'bg-red-500' : 'bg-green-500' },
    { title: t('inventory.inventoryValue'), href: '/dashboard/inventory', icon: TrendingDown, value: `ETB ${totalValue.toFixed(0)}`, color: 'bg-purple-500' },
    { title: t('inventory.suppliers'), href: '/dashboard/inventory/suppliers', icon: Truck, color: 'bg-orange-500' },
    { title: t('inventory.purchases'), href: '/dashboard/inventory/purchases', icon: ShoppingCart, color: 'bg-teal-500' },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      {alerts.length > 0 && (
        <Card className="border-red-200 bg-red-50">
          <CardHeader className="flex flex-row items-center gap-2 pb-2">
            ...
            <CardTitle className="text-sm text-red-700">{t('inventory.lowStockAlerts')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {alerts.slice(0, 5).map(alert => (
              <div key={alert.id} className="flex justify-between text-sm">
                <span>{alert.ingredient?.name || t('inventory.unknown')}</span>
                <span className="text-red-600 font-medium">{t('inventory.remaining', { count: alert.current_quantity, min: alert.reorder_level })}</span>
              </div>
            ))}
          </CardContent>
        </Card>
            ...
            <CardTitle className="text-base flex items-center gap-2">
              ...
              <Link href="/dashboard/inventory/purchases">
                <Button variant="ghost" size="sm" className="gap-1 text-xs">
                  {t('inventory.viewAll')} <ArrowRight className="h-3 w-3" />
                </Button>
              </Link>
          </CardHeader>
          <CardContent>
            {purchases.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">{t('inventory.noPurchases')}</p>
            ) : (
                ...
                {purchases.slice(0, 5).map(po => (
                  <div key={po.id} className="flex items-center justify-between py-1.5 border-b last:border-0 text-sm">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{po.supplier?.name || t('inventory.unknown')} &middot; {formatDate(po.order_date)}</p>
                    </div>
            ...
            {wastage.length > 0 && (
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="rounded-lg bg-muted/50 p-3 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">{t('inventory.totalUnitsWasted')}</p>
                </div>
                <div className="rounded-lg bg-muted/50 p-3 text-center">
                  <p className="text-2xl font-bold">{wastage.length}</p>
                  <p className="text-xs text-muted-foreground">{t('inventory.wastageRecords')}</p>
                </div>
              </div>
            )}
            {wastage.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">{t('inventory.noWastage')}</p>
            ) : (
              <div className="space-y-2">
                {wastage.slice(0, 5).map(w => (
                  <div key={w.id} className="flex items-center justify-between py-1.5 border-b last:border-0 text-sm">
                    <div>
                      <span className="font-medium">{w.ingredient?.name || t('inventory.unknown')}</span>
      ...
      <Card>
        <CardHeader><CardTitle>{t('inventory.currentStock')}</CardTitle></CardHeader>
        <CardContent>
          <div className="space-y-2">
          ...
          {stock.length === 0 && <p className="text-muted-foreground text-sm">{t('inventory.noStock')}</p>}
        </CardContent>
      </Card>
    </div>
  )
